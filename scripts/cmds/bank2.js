...
config: {
  ...
  const p = getPrefix(event.threadID);
    ...
    const lianeBank = "💰 بنك يوتا "; //do not change
 ...
 const command = args[0];
  ...
  if (userDice > cassidyBotDice) {
    ...
    outcomeMessage += `يوتا البوت ربح لتو 100 دولار مع النتيجة ${cassidyBotDice}.`;
    bankData[user].bank -= 100;
  } else {
  ...
  if (args[1] === "تأكيد") {
    ...
    if (successRate < 0.7) {
      ...
      return message.reply(`${lianeBank}\n\n✧ عفوًا، لقد تم القبض عليك., ${userName}! لقد كانت سرقة البنك الخاص بك غير ناجحة. لا يمكنك سرقة أي شيء هذه المرة. لكن, 10% تمت إضافة إجمالي مبلغ السرقة إلى حسابك في بنك يوتا , ${loanAmount} تم خصمها من رصيدك البنكي`);
    }
